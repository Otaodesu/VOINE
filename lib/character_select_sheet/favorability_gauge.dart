...
void incrementSpeakerUseCount({required int speakerId}) async {
  final speakerIdUseCountMap = await loadSpeakerIdUseCountMap();
...
// テイクアウトで作り置きせず必要になるたび生成するようにしてみた
class SpeakerFavorabilityGauge extends StatelessWidget {
  const SpeakerFavorabilityGauge({super.key, required this.speakerUseCount});

  final int speakerUseCount;

  @override
  Widget build(BuildContext context) {
    final speakerFavorabilityRateMax5 = 5 * speakerUseCount / (4 + speakerUseCount); // ☆の数を算出。1回使用で☆1にしたい.
    ...
    return Transform.flip(
      ...
      child: RatingBar(
        ...
        ratingWidget: RatingWidget(
          full: Icon(Icons.star_rounded, color: Colors.lightGreen.shade400),
          half: Icon(
            Icons.star_half_rounded,
            color: Colors.lightGreen.shade400,
          ), // 色決めるのムズ！異物感がなく文字より目立たずアイコンにマッチするもの…🤯.
...
Future<Map<int, int>> loadSpeakerIdUseCountMap() async {
