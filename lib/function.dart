...
/// ユーザーアイコンをpickして永続的なフォルダに保存する関数。選択せずに閉じたらnullを返す。
Future<File?> pickUserIcon() async {
  final pickResult = await ImagePicker().pickImage(
    source: ImageSource.gallery,
    maxWidth: 500, // アイコンならwidth減らしても十分ちゃう？
  );
  if (pickResult == null) {
    return null;
  }
  print('${DateTime.now()}🤘コピーを開始します！');

  final bytes = await pickResult.readAsBytes();
  final digest = md5.convert(bytes);
  final hash = digest.toString(); // ファイル名そのままでコピーするとダブったときエラいことになりそうなのでハッシュ値にした

  final appDir = await getApplicationDocumentsDirectory();
  final extension = p.extension(pickResult.path); // FileImageは拡張子がなくても表示できるっぽいけど一応

  final savedImage = File('${appDir.path}/$hash$extension');
  await pickResult.saveTo(savedImage.path);

  print('${DateTime.now()}🦊${savedImage.path}に保存しました！');
  return savedImage;
}
