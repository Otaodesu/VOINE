...
class FukidashiLongPressDialog extends StatelessWidget {
  const FukidashiLongPressDialog({
    ...
    required this.onChangeIconPressed,
  });
  ...
  final VoidCallback onChangeIconPressed;
  ...
  Widget build(BuildContext context) => SimpleDialog(
    ...
    children: [
      SimpleDialogOption(
        ...
        child: const ListTile(leading: Icon(Icons.delete_rounded), title: Text('削除する')),
      ),
      SimpleDialogOption(
        onPressed: () {
          Navigator.of(context).pop();
        ...
        child: const ListTile(leading: Icon(Icons.move_up_rounded), title: Text('上に移動する')),
      ),
      SimpleDialogOption(
        onPressed: () {
          Navigator.of(context).pop();
        ...
        child: const ListTile(leading: Icon(Icons.move_down_rounded), title: Text('下に移動する')),
      ),
      SimpleDialogOption(
        onPressed: () {
          Navigator.of(context).pop();
        ...
        child: const ListTile(leading: Icon(Icons.playlist_play_rounded), title: Text('ここから連続再生する')),
      ),
      SimpleDialogOption(
        onPressed: () {
          Navigator.of(context).pop();
        ...
        child: const ListTile(leading: Icon(Icons.graphic_eq_rounded), title: Text('この先すべてを音声合成する')),
      ),
      SimpleDialogOption(
        onPressed: () {
          Navigator.of(context).pop();
        ...
        child: const ListTile(leading: Icon(Icons.account_circle_rounded), title: Text('アイコンを変更する')),
      ),
      SimpleDialogOption(
        onPressed: () {
          Navigator.of(context).pop();
        ...
        child: const ListTile(leading: Icon(Icons.add_comment_rounded), title: Text('セリフを追加する')),
      ),
      SimpleDialogOption(
        onPressed: () {
          Navigator.of(context).pop();
...
class AppBarForChat extends StatelessWidget implements PreferredSizeWidget {
  const AppBarForChat({
    ...
    required this.onHamburgerPress, // 🍔はプレスするものだからPress.
  });
  ...
  Widget build(BuildContext context) => AppBar(
    ...
    actions: [
      ...
      Tooltip(message: '再生を停止する', child: IconButton(icon: const Icon(Icons.stop_rounded), onPressed: onStopTap)),
      Tooltip(
...
class HamburgerMenuForChat extends StatelessWidget {
  const HamburgerMenuForChat({
    super.key,
    required this.onExportProjectPressed,
    required this.onExportAsTextPressed,
    required this.onDeleteAllMessagesPressed,
    required this.onImportProjectPressed,
    required this.onEditTextDictionaryPressed,
  });
  ...
  Widget build(BuildContext context) => SimpleDialog(
    ...
    children: [
      SimpleDialogOption(
        ...
        child: const ListTile(leading: Icon(Icons.list_alt_rounded), title: Text('テキストとして書き出す（.txt）')),
      ),
      SimpleDialogOption(
        onPressed: onExportProjectPressed,
        child: const ListTile(leading: Icon(Icons.output_rounded), title: Text('プロジェクトを書き出す（.zrproj）')),
      ),
      SimpleDialogOption(
        onPressed: onImportProjectPressed,
        child: const ListTile(leading: Icon(Icons.exit_to_app_rounded), title: Text('プロジェクトを読み込む（.zrproj）')),
      ),
      SimpleDialogOption(
        onPressed: onEditTextDictionaryPressed,
        child: const ListTile(leading: Icon(Icons.import_contacts_rounded), title: Text('読み方辞書を開く')),
      ),
      SimpleDialogOption(
        onPressed: onDeleteAllMessagesPressed,
        child: const ListTile(leading: Icon(Icons.delete_forever_rounded), title: Text('すべて削除する')),
      ),
    ],
  );
}
...
class AlternateOfKakidashi extends StatelessWidget {
  ...
  Widget build(BuildContext context) => AlertDialog(
    title: Text(whatYouWantSetTitle, overflow: TextOverflow.fade, softWrap: false),
    content: SelectableText(whatYouWantShow, style: const TextStyle(fontSize: 15)),
    actions: [
      Tooltip(
        message: 'すべてコピーする',
        child: IconButton(onPressed: _saveOnClipboard, icon: const Icon(Icons.copy_rounded)),
      ),
      Tooltip(
        message: 'ファイルにして保存する',
        child: IconButton(onPressed: _shareAsFile, icon: const Icon(Icons.folder_outlined)),
      ),
    ],
  );
          ...
          Row(
            children: [
              Tooltip(
                ...
                child: IconButton(onPressed: _loadFromClipboard, icon: const Icon(Icons.paste_rounded)),
              ),
              Tooltip(
                message: 'ファイルを読み込む',
                child: IconButton(
